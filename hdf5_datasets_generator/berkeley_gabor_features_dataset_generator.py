...
def get_gabor_features(img_complex, gabor_filters, r_type, gsmooth, opn, selem_size):
    ...
    # ##  No Parseval's identity
    g_features = []
    for ff in range(filter_responses.shape[1]):
        g_features.append(reshape4clustering(filter_responses[0][ff], rows, cols))
        g_features.append(reshape4clustering(filter_responses[1][ff], rows, cols))
        g_features.append(reshape4clustering(filter_responses[2][ff], rows, cols))
...
def generate_h5_features_dataset(num_imgs, periods, bandwidths, crossing_points, deviations, colorspaces):
    ...
    for (min_period, max_period), (fb, ab), (c1, c2), stds, color_space in itertools.product(periods, bandwidths,
                                                                              crossing_points, deviations, colorspaces):
        ...
        gabor_conf = ('%df_%da_%dp_%dp_%.2ffb_%dab_%.2fcpf_%.2fcpa_%.1fstds_%s'
                      % (n_freq, n_angles, min_period, max_period, fb, ab, c1, c2, stds, color_space))
        print('Computing Gabor features: ' + gabor_conf)
        t0 = time.time()

        # ## No Parseval's identity
        twoChannel_imgs = Parallel(n_jobs=num_cores)(
            delayed(img2complex_colorspace)(img, color_space) for img, shape in
        ...
        with ImageIndexer(h5_outdir / 'Gabor_features.h5',
            ...
            for features in gabor_features:
                ...
                imageindexer.db.attrs['color_space'] = color_space
