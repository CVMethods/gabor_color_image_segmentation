...
def get_normalized_cuts_metrics(im_file, img, regions_slic, graph_raw, perceptual_gradients, graph_mode, aff_norm_method, gradients_dir, outdir):
    print('##############################', im_file, '##############################')
...
def normalized_graphcut_segmentation(num_imgs, n_slic, graph_type, similarity_measure, gradients_dir, graph_mode,
                                     aff_norm_method):
    num_cores = -1
    ...
    if gradients_dir == 'predicted_gradients':
        ...
        for mm, model_name in enumerate(model_input_dirs):
            ...
            for gradients_input_file in input_files:
                with h5py.File(hdf5_indir_grad / model_name / gradients_input_file / 'predicted_gradients.h5',
                    ...
                    outdir = '../outdir/' + \
                             ...
                             (str(n_slic) + '_slic_' + graph_type + '_' + similarity_measure) + '/' + \
                             model_name + '/' + \
                             'normalized_graphcut_' + aff_norm_method + '_norm_' + graph_mode + '_graph/' + \
                             gradients_input_file + '/'

                    metrics_values = Parallel(n_jobs=num_cores)(
                        delayed(get_normalized_cuts_metrics)(im_file, img, regions_slic, graph_raw,
            ...
            plt.grid()
            plt.savefig(
                outdir + 'Normalized_graphcut_precisions_' + aff_norm_method + '_' + graph_mode + '_boxplot.png',
                bbox_inches='tight')
        ...
        for gradients_input_file in input_files:
            with h5py.File(hdf5_indir_grad / gradients_input_file/ 'gradients.h5', "r+") as gradients_file:
                ...
                metrics_values = Parallel(n_jobs=num_cores)(
                    delayed(get_normalized_cuts_metrics)(im_file, img, regions_slic, graph_raw, perceptual_gradients,
                                                             graph_mode, aff_norm_method, gradients_dir, outdir)
                    for im_file, img, regions_slic, graph_raw, perceptual_gradients in
                    zip(img_ids, images, superpixels, raw_graphs, imgs_gradients))
