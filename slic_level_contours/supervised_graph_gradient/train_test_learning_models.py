...
def predicted_gradient_computation(im_file, img_shape, edges_info, perceptual_gradients, model, sclr, outdir, outdir_cont):
    print('##############################', im_file, '##############################')
...
def train_test_models(num_imgs, similarity_measure, kneighbors=None, n_slic=None, graph_type=None):
    ...
    if kneighbors is not None:
        ...
        hdf5_outdir = Path(source_dir + '../../../data/hdf5_datasets/' + str(
            num_imgs) + 'images/' + 'predicted_gradients/' + final_dir)
            ...
            with h5py.File(hdf5_indir_grad / gradients_input_dir / 'gradients.h5', "r+") as gradients_file:
                ...
                predicted_gradients = Parallel(n_jobs=num_cores)(
                    delayed(predicted_gradient_computation)(im_file, img_shape, edges_info,
                                                            ...
                                                            regressor, scaler, outdir, outdir2)
        ...
        if 'val' in np.unique(img_subdirs):
            pass
        else:
            i_val = np.random.choice(np.where(img_subdirs == 'train')[0], size=50)
            img_subdirs[i_val] = 'val'
        ...
        for gradients_input_dir in input_directories:
            with h5py.File(hdf5_indir_grad / gradients_input_dir / 'gradients.h5', "r+") as gradients_file:
                ...
                scaler = MinMaxScaler() # None  # StandardScaler()  #
                ...
                for name, regressor in regressors:
                    ...
                    if name == 'SimpleSum':
                        ...
                        es = EarlyStopping(monitor='val_loss', mode='min', verbose=0, patience=20, min_delta=0.1, restore_best_weights=True)
